...
Ebben a fejezetben először hőtani alapösszefüggéseket ismertetek, amelyekből előáll majd a fűtőtestek teljesítményét leíró modell. Majd a modell által számolt teljesítményt egy Simscape-ben megvalósított termikus hálózatra vezetem, ami a fűtőtest tranziens viselkedését adja meg. Így szimulálható a szoba felfűtése
...
A hőnyereség számításakor a be- és kikapcsolási tranzienseket is figyelembe kell venni, ez az az idő, ami alatt a hőtároló elemek beállnak az állandósult állapotbeli hőmérsékletükre.
A fenti két megoldás máshogy fejti ki hatását Szimuláció során olyan bekapcsolási tranziensekkel is számolnom kell, amik egy kis időállandós fűtés (pl. légbefúvás, fan-coil) esetén elhanyagolhatók lennének.
...
A fűtőtestek hőleadását befolyásolja a fűtőtestek közepes hőmérsékletkülönbsége (ld. a \ref{eq_termeszeteshk_359}. egyenletet), a felülete és a hőleadási tényezője.
...
%
Ezek közötti kapcsolatot adja az \ref{eq_holeadas}. egyenlet (\cite[358.~o.]{Herz}-ből): 
...
\begin{aligned}
\Delta t_m = t_s - t_i \\[10pt]
t_s = \frac{t_w+t_r}{2} -t_{drop}
\end{aligned}
\end{equation}
ahol \si{\celsius}-ban szerepelnek:
%
\begin{itemize}[itemsep=6pt,topsep=0pt,parsep=0pt,partopsep=0pt]
	\item[] $t_s$ a fűtőtest felületi hőmérséklete
	\item[] $t_i$ a szoba hőmérséklete
	\item[] $t_w$ a radiátorba befolyó\footnote{Consider using $T_w$ for supply water temp instead of $T_s$ to avoid misinterpretation as sampling time.}, $t_r$ az onnan kifolyó víz hőmérséklete
...
\begin{aligned}
\dot Q_{le} ~=~ & k_e ~ A_e ~ \left( t_s-t_i-\frac{\dot Q_{fel}}{2~c ~ \dot{m}}\right)  \\[18pt]
\dot Q_{le} + \frac{k_e ~ A_e ~ \dot Q_{fel}}{2 ~ c ~ \dot{m}} ~ = ~ & k_e ~ A_e ~(t_s-t_i) \\[24pt]
2 ~ c ~ \dot{m} ~ \dot Q_{le} + k_e ~ A_e ~ \dot Q_{fel} ~ = ~ &  k_e ~ A_e ~ 2~ c~ \dot{m} ~(t_w-t_i)
\end{aligned}
\end{equation}
...
\begin{aligned}
~~~~~~\dot Q (2 ~ c ~ \dot{m} + k_e ~ A_e) & ~=~ 2 ~ k_e ~ A_e ~ c~ \dot{m} ~(t_w-t_{drop}-t_i) \\[18pt]
...
\begin{formal}
	\textbf{Megjegyzés:} A radiátorba bekerülő teljesítményt a $t_w-t_r$ szabja meg (\ref{eq_hofelvetel}. egyenlet), viszont itt $t_r$-t kiejtettem az egyenletekből. Viszonta REHVA Guidebook
...
\section{Dinamikus hőátadás modellje}
...
A radiátorokat mindig az adott helyiséghez méretezik, ezért az adatokat leolvasással / katalógusból kapjuk normál esetben. A modellezéshez választanom kellett egy típust. Itt még csak paraméteresen kellene megadni az értékeket, avgy előbb a ház modelljét, hőszükségletét felírni, elvégre is a házhoz tervezzü
